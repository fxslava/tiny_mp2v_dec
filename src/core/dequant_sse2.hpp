...
MP2V_INLINE void dequant_sse2(int16_t* QFS, int8_t qfs[64], int8_t qid[64], int n, uint16_t W[64], uint8_t quantizer_scale) {
    ALIGN(16) int16_t tmp[8];
    int i = 0;
    if (n & ~7)
        for (; i < (n & ~7); i += 8, qid += 8) {
            __m128i vqfs = _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i*) & qfs[i]), _mm_setzero_si128());
            _mm_store_si128((__m128i*) tmp, inverse_quant_scalar_sse2<intra>(vqfs, _mm_mullo_epi16(_mm_load_si128((__m128i*) & W[i]), _mm_set1_epi16(quantizer_scale))));
            for (int j = 0; j < 8; j++) {
    ...
    if (n & 7) {
        __m128i vqfs = _mm_unpacklo_epi8(_mm_load_si128((__m128i*) & qfs[i]), _mm_setzero_si128());
        _mm_store_si128((__m128i*) tmp, inverse_quant_scalar_sse2<intra>(vqfs, _mm_mullo_epi16(_mm_load_si128((__m128i*) & W[i]), _mm_set1_epi16(quantizer_scale))));
        for (int j = 0; j < (n & 7); j++) {
            int idx = (int)g_scan_trans[alt_scan ? 1 : 0][qid[j]];
            QFS[idx] = tmp[j];
